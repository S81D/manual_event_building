...
def submit_grid_job(run, p_start, p_end, input_path, output_path, TA_tar_name, disk_space, source):
    ...
    # FNAL-only nodes
    #file.write('jobsub_submit --memory=2000MB --expected-lifetime=2h -G annie --disk=' + disk_space + 'GB --resource-provides=usage_model=DEDICATED,OPPORTUNISTIC --singularity-image /cvmfs/singularity.opensciencegrid.org/fermilab/fnal-wn-sl7:latest ')
...
def grid_job(run, user, input_path, TA_tar_name, name_TA, source):
    ...
    # NEW - to ensure singularity bind mounting is working
    file.write('\n')
    ...
    file.write('rm /srv/Trig* \n')                 # For source this won't be here, but just to make sure
...
def run_container_job(run, name_TA, source):
